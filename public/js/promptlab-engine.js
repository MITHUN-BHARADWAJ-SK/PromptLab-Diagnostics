...
/**
 * PromptLab — Template Generator Service (Pivot v1)
 *
 * Role: Highly skilled prompt generator for content creators and students.
 * Architecture: Simplified meta-prompt (template) generator.
 * Output: Versatile, fill-in-the-blank templates for reuse.
 */
...
const PERSONA = `You are a highly skilled prompt generator, designed to assist content creators and students in crafting a wide range of prompts tailored to their specific needs. Your expertise lies in creating prompts that can be saved and reused, enhancing the productivity and creativity of users.`;
...
/**
 * Generate a versatile prompt template.
 *
 * @param {Object} params
 ...
 */
function generate({ promptText, modelTarget }) {
    if (!promptText || typeof promptText !== 'string' || promptText.trim().length === 0) {
    ...
    const v1Template = _createTemplate(fields);

    // 3. Refine for clarity/accessibility (V2)
    const v2Template = _refineTemplateAccessiblity(v1Template, fields);

    return {
        finalPrompt: v2Template || v1Template,
        v1: v1Template,
        v2: v2Template,
        route: 'template_generator',
        ambiguityScore: 0, // No longer using ambiguity scoring for routing
        _fields: fields
    };
}
...
function _extractTemplateFields(text) {
    const lower = text.toLowerCase();

    // Type of content
    let type = '[type of content]';
    const typeMatches = [
        { key: 'blog post', r: /\b(blog|article|post)\b/i },
        { key: 'essay', r: /\b(essay|paper|academic)\b/i },
        { key: 'social media post', r: /\b(social|tweet|instagram|caption)\b/i },
        { key: 'email', r: /\b(email|letter|message)\b/i },
        { key: 'poem', r: /\b(poem|poetry|sonnet|haiku)\b/i },
        { key: 'code', r: /\b(code|function|script|program)\b/i },
        { key: 'story', r: /\b(story|fiction|narrative)\b/i }
    ];
    ...
    const audienceMatches = [
        { key: 'content creators', r: /\b(creator|influencer|blogger|marketing)\b/i },
        { key: 'students', r: /\b(student|learner|academic|researcher)\b/i },
        { key: 'professionals', r: /\b(professional|expert|senior|business)\b/i },
        { key: 'beginners', r: /\b(beginner|novice|entry-level)\b/i }
    ];
    ...
    const subjectMatch = text.match(/\b(?:about|regarding|on|related to|concerning|subject)\s+(.{3,60}?)(?:\.|,|\?|$|\band\b|for\b|with\b)/i);
    if (subjectMatch) {
        topic = subjectMatch[1].trim();
    } else {
        // Fallback: use first few meaningful words if no "about" phrase
        const words = text.split(/\s+/).filter(w => w.length > 3 && !['write', 'make', 'create', 'generate'].includes(w.toLowerCase()));
    ...
    let tone = '[tone]';
    const toneMatches = [
        { key: 'professional', r: /\b(professional|formal|serious)\b/i },
        { key: 'casual', r: /\b(casual|friendly|fun)\b/i },
        { key: 'academic', r: /\b(academic|scholarly|educational)\b/i },
        { key: 'persuasive', r: /\b(persuasive|sales|convincing)\b/i },
        { key: 'creative', r: /\b(creative|whimsical|imaginative)\b/i }
    ];
    ...
    return { type, audience, topic, tone, original: text };
}

function _createTemplate(fields) {
    const lines = [];

    // Header based on user instructions
    lines.push(`# Role`);
    lines.push(`${PERSONA}`);
    lines.push('');

    lines.push(`# Task`);
    lines.push(`Generate a reusable template for ${fields.type} about ${fields.topic} targeted at ${fields.audience}.`);
    lines.push('');

    lines.push(`# Template`);
    lines.push(`"Write a [${fields.type}] about [${fields.topic}] for [${fields.audience}]. The tone should be [${fields.tone}], and it should include [specific elements]."`);
    lines.push('');

    lines.push(`# Guidelines`);
    lines.push(`- Ensure the tone remains [${fields.tone}] throughout.`);
    lines.push(`- Include key elements relevant to [${fields.topic}].`);
    lines.push(`- Structure clearly for easy modification by [${fields.audience}].`);
    lines.push(`- Avoid overly complex language or jargon.`);

    return lines.join('\n');
}

function _refineTemplateAccessiblity(template, fields) {
    // Ensure "no jargon" and "accessible" by replacing complex terms
    ...
    for (const [jargon, simple] of Object.entries(simpleTerms)) {
        const regex = new RegExp(`\\b${jargon}\\b`, 'gi');
        refined = refined.replace(regex, simple);
    }
