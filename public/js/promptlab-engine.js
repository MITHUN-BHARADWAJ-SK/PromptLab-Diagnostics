...
/**
 * Refine a template-generated prompt using GPT-4o-mini.
 * - Only runs server-side (Node.js with OPENAI_API_KEY env var)
                ...
                (res) => {
                    ...
                    res.on('end', () => {
                        try {
                            ...
                            resolve(content || templatePrompt);
                        } catch {
                            resolve(templatePrompt);
                        }
                    });
                }
            );
            req.on('error', () => resolve(templatePrompt));
            req.setTimeout(8000, () => { req.destroy(); resolve(templatePrompt); });
            req.write(payload);
            req.end();
        });
