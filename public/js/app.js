...
async function optimizePrompt() {
  ...
  try {
    // Check quota (paid = generation counter; free = credits)
    const isPaidOpt = state.userTier.toLowerCase() !== 'free';
    if (isPaidOpt) {
      const quota = await PromptLabDB.checkGenerations(state.uid);
      if (!quota || quota.remaining <= 0) {
        showToast('Monthly generation limit reached. Upgrade your plan.', 'error');
        return;
      }
    } else {
      const credits = await PromptLabDB.checkCredits(state.uid);
      if (credits.total < 3) {
        showToast('Insufficient credits (3 required for optimization). Earn more with daily login.', 'error');
        return;
      }
    }
    ...
    if (isPaidOpt) {
      await PromptLabDB.consumeGeneration(state.uid);
    } else {
...
async function generatePrompt() {
  ...
  try {
    ...
    const isPaid = state.userTier.toLowerCase() !== 'free';
    if (isPaid) {
      const quota = await PromptLabDB.checkGenerations(state.uid);
      if (!quota || quota.remaining <= 0) {
        showToast('Monthly generation limit reached. Upgrade your plan.', 'error');
      ...
      const credits = await PromptLabDB.checkCredits(state.uid);
      if (credits.total < 3) {
        showToast('Insufficient credits (3 required). Earn more with daily login.', 'error');
...
function renderHistory(entries) {
  ...
  // Event delegation — avoids inline onclick with interpolated IDs
  if (!tbody._historyClickBound) {
    tbody.addEventListener('click', e => {
      const card = e.target.closest('[data-history-id]');
      if (card) viewHistoryItem(card.dataset.historyId);
    });
    tbody._historyClickBound = true;
  }
  ...
  tbody.innerHTML = entries.map(p => {
    ...
    return `
        <div class="group bg-white dark:bg-[#191919]/40 border border-slate-200 dark:border-white/10 hover:border-primary/40 rounded-xl p-4 flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6 transition-all duration-300 cursor-pointer glow-border terminal-glass"
             data-history-id="${escapeHtml(p.id)}">
...
async function refreshCredits() {
  ...
  try {
    ...
    const isPaid = tier.toLowerCase() !== 'free';
    const headerPill = document.getElementById('headerProfilePill');
    ...
    if (isPaid) {
      ...
      if (headerPill) {
        headerPill.style.display = 'flex';
        if (roleEl) roleEl.textContent = `${tier} · ${remaining} Generations Left`;
      }
      if (dashRemaining) dashRemaining.textContent = remaining;
      if (dashRemainingLabel) dashRemainingLabel.textContent = 'GENERATIONS LEFT';
    } else {
      const credits = await PromptLabDB.checkCredits(state.uid);
      const periodLabel = credits.period === 'day' ? 'Daily' : 'Monthly';
      if (headerPill) {
        headerPill.style.display = 'flex';
        if (roleEl) roleEl.textContent = `${tier} · ${credits.total} ${periodLabel} Credits`;
      }
      if (dashRemaining) dashRemaining.textContent = credits.total;
      if (dashRemainingLabel) dashRemainingLabel.textContent = `${periodLabel.toUpperCase()} CREDITS`;
    }
...
function escapeHtml(str) {
  ...
  return String(str)
    ...
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');  // safe in both HTML and JS attribute contexts
}

function formatExplanation(text) {
  if (typeof text !== 'string') return '';
  // escapeHtml runs FIRST so $1 captures are already entity-encoded —
  // the resulting <strong> tags are safe static markup, not user data.
...
// Apply upgraded tier to Firestore — updates subscriptionTier and resets generation quota
async function _applyUpgradedTier(tier) {
  const tierGenerations = {
    starter: 200,
    pro: 1000,
    advanced: 3000,
    builder: 5000,
  ...
  const limit = tierGenerations[tier];
  const update = { subscriptionTier: tier };
  if (limit) {
    update.monthlyGenerations = limit;
    update.monthlyGenerationReset = new Date(new Date().setMonth(new Date().getMonth() + 1, 1)).toISOString();
  }
